...
public class CropImageView extends View {
    ...
    public Bitmap getCropImage() {
        ...
        mDrawable.draw(canvas);
        if (mDrawableFloat.top + mDrawableFloat.height() > getHeight()) {
            tmpBitmap = createBitmap(getWidth(), mDrawableFloat.top + mDrawableFloat.height());
        } else if (mDrawableFloat.left <= 0) {
            mDrawableFloat.left = 2;
        } else if (mDrawableFloat.left + mDrawableFloat.width() > getWidth()) {
            tmpBitmap = createBitmap(mDrawableFloat.left + mDrawableFloat.width(), getHeight());
        }

        Bitmap ret =null;
        if ( mDrawableFloat.left +mDrawableFloat.width() > tmpBitmap.getWidth() ){
            Toast.makeText(mContext, R.string.tip_edit_picture_fail,Toast.LENGTH_SHORT).show();
        }else{
            ret = cropBitmap(tmpBitmap, mDrawableFloat.width(), mDrawableFloat.height());
        }
        tmpBitmap.recycle();
        tmpBitmap = null;
        return ret;
    ...
    private Bitmap cropBitmap(Bitmap cropBitmap, int width, int height) {
        Matrix matrix = new Matrix();
        float scale = (float) (mDrawableSrc.width())
                / (float) (mDrawableDst.width());
        matrix.postScale(scale, scale);
        return Bitmap.createBitmap(cropBitmap, mDrawableFloat.left,
                mDrawableFloat.top, width, height, matrix, true);
    }
